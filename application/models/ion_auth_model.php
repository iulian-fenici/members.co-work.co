...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function email_check($email = '', $ignoreNotActive = false)
	{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $groups = array(), $ignoreNotActive = false)
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation', 'ion_auth');

		if ($this->identity_column == 'email' && $this->email_check($email, $ignoreNotActive))
		{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->trigger_events('extra_where');
                $this->db->where('active != 0'); //Ignore not active users. Check active users only.
		$query = $this->db->select('u.'.$this->identity_column . ', u.username, u.email, u.id, u.password, u.active, u.last_login, u.company_id')
                          ->from($this->tables['users'].' u')
                      //    ->join('companies c','c.id = u.company_id','left')
		                  ->where('u.'.$this->identity_column, $this->db->escape_str($identity))
		                  ->limit(1)
		                  ->get();
	...
	**/
	...
	public function login_remembered_user()
	{
        ...
        $query = $this->db->select('u.'.$this->identity_column . ', u.username, u.email, u.id, u.password, u.active, u.last_login, u.company_id')
                          ->from($this->tables['users'].' u')                         
		                  ->where('u.'.$this->identity_column, get_cookie('identity'))
