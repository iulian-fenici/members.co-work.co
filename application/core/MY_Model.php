...
class MY_Model extends CI_Model {
    ...
    public function soft_delete($id,$key='deleted') {
        $this->trigger('before_delete', $id);

        $this->db->where($this->primary_key, $id);

        $result = $this->db->update($this->_table, array($key => TRUE));
    ...
    public function rename_column($column_name, $table = null) {
        $name = null;
        $columns = $this->config->item($table, 'db_filters');
        if (isset($columns[$column_name]))
            $name = $columns[$column_name];
        return $name;
    }

    public function apply_filters($filters, $table = null) {
        if ($filters) {
            foreach ($filters as $k => $v) {
                if (in_array($k, array('order', 'direction', 'limit', 'offset')))
                    continue;
                $type = $this->config->item($k, 'db_filters_rules');
                $column_name = $this->rename_column($k, $table);

                if ($column_name) {
                    if ($type == 2 && is_array($column_name)) {// type == 2
                        $likeArr = array();
                    ...
                    } else if ($type == 1 || !preg_match('/^([\w\._]+)$/', $column_name)) {
                        $this->db->where($column_name, $v);
                    } else if ($type == 3) { //type = 3 >=
                        $this->db->where($column_name.' >=', $v);
                    }else if ($type == 4) { //type = 3 <=
                        $this->db->where($column_name.' <=', $v);
                    }else { // type == 0
                        $this->db->like($column_name, $v);
                    }
                }
            }
        }
    ...
    public function apply_order($filters, $table = null, $default = false) {
        ...
        foreach ($filters as $k => $v) {
            if ($k) {
                if (is_array($v)) {
                    foreach ($v as $k1 => $v1) {
                        $name = $this->rename_column($k1, $table);
                        if ($name)
                            $this->db->order_by($name, $v1);
                    }
                }
                else {
                    $name = $this->rename_column($k, $table);
                    if ($name)
                        $this->db->order_by($name, $v);
                }
            }
        }
    }
