<script type="text/javascript">
    ...
    function deleteDeskAssignment(url){
        if(confirm('Are you sure you want delete this assignment?')){
            $.ajax({
                url: url,
                type: "POST",
                dataType: 'json',
                ...
                success: function(ret){
                    if(ret.success != undefined)
                    {
                        $('#'+ret.desk_id).attr('desk_number', "");
                        $('#'+ret.desk_id).attr('desk_comment',"");
                        $('#'+ret.desk_id).attr('company_id', "");
                        //$('#'+ret.desk_id).attr('cd_id', "");
                        $('#'+ret.desk_id).attr('company', "");
                        hilightMapRegions();
                        $.fancybox.close();
                    ...
                    {
                        setError($('#editDeskErrorBlock'),ret.message,'error');
                    }
                }
            });
        }        
    }
    function editDeskAssignment(url){
        if(confirm('Are you sure you want update this assignment?')){
    ...
    <div class="span12">
        <div class="edit-desk-container">
            <div class="row-fluid">
                ...
                <div class="row-fluid">
                    <span class="btn btn-success pull-left" onclick="editDeskAssignment('<?php echo '/admin/plans/editDeskAssignment/'.$deskData['id']; ?>')">Save</span>
                    <span class="btn btn-danger pull-right" onclick="deleteDeskAssignment('<?php echo '/admin/plans/deleteDeskAssignment/' . $deskData['id']; ?>');">Release</span>
                </div>
            </div>
        </div>
    </div>
