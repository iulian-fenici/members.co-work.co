...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        <?php if($this->input->get('date')):?>
            changeMonthFilter('date','<?php echo $this->input->get('date');?>');
        <?php else:?>
            loadDayFilter();   
        <?php endif;?>
    ...
    function loadCalendar(date,dateString){
        $.ajax({
            url: "/merchant/dashboard/get_month_calendar",
            type: "POST",
            dataType: 'json',
            cache: false,
            async: true,
    ...
    function setTooltipText(event){
        var template = 
            '<div>'+
                ...
                (typeof event.location_name ==='undefined' || event.location_name==''?'':'<span><b>Location name:</b> </span>'+event.location_name+'<br>')+
    ...
    function dayClick(date){
        ...
        window.location.href = '/merchant/dashboard/index?date='+str;
    }
</script>
